      ...
      <form onSubmit={handleSubmit}>
        ...
        <label>Þjóðerni:
          <select name="nationality" value={formData.nationality} onChange={handleChange} required>
            ...
            {countries.map(c => (
              <option key={c.country_code} value={c.country_code}>{c.name_en}</option>
            ))}
          </select>
        </label>
        ...
        <label>Vænghaf (cm):
          <input type="number" name="wingspan_cm" value={formData.wingspan_cm} onChange={handleChange} />
        </label>

        <div style={{ marginTop: '1rem' }}>
          <button type="submit">Vista</button>
          <button type="button" onClick={onCancel} style={{ marginLeft: '1rem' }}>Hætta við</button>
        </div>
      </form>
    </div>
  );
