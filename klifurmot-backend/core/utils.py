...
def error_response(
    ...
    Args:
        ...
        details: Optional list of detailed error messages or dictionary of field errors
        status_code: HTTP status code
...
def validation_error_response(serializer_errors: Dict[str, Any]) -> Response:
    """
    ...
    if not hasattr(serializer_errors, "items"):
    ...
    for field, errors in serializer_errors.items():
        if isinstance(errors, list):
            ...
            if any(
                "already exists" in str(e).lower() or "already in use" in str(e).lower()
                for e in errors
            ):
            ...
            details[field] = [errors]
            if (
                "already exists" in str(errors).lower()
                or "already in use" in str(errors).lower()
            ):
                duplicate_detected = True
