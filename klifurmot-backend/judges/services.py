...
def claim_invitation(token: str, user: User = None) -> Dict[str, Any]:
...
def get_competition_invitations(competition_id: int, user: User) -> Dict[str, Any]:
    ...
    if not user_account.is_admin:
        ...
        if not is_competition_admin:
            raise PermissionError('You do not have permission to view invitations for this competition')
...
def create_judge_link(competition_id: int, user: User, user_id: int, expires_at: timezone.datetime) -> Dict[str, Any]:
...
def get_competition_judge_links(competition_id: int, user: User) -> Dict[str, Any]:
    ...
    if not user_account.is_admin:
        ...
        if not is_competition_admin:
            raise PermissionError('You do not have permission to view judges for this competition')
...
def update_judge_link(link_id: int, user: User, expires_at: timezone.datetime) -> Dict[str, Any]:
    ...
    if not user_account.is_admin:
        ...
        if not is_competition_admin:
            raise PermissionError('You do not have permission to manage this judge link')
