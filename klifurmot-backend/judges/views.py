...
def send_invitation(request, competition_id):
    ...
    try:
        ...
        return utils.success_response(
            ...
            message='Invitation sent successfully' if result['type'] == 'new_user' else 'Judge link created for existing user',
...
def claim_invitation(request, token):
    ...
    try:
        ...
        if not result['authenticated']:
            return utils.success_response(
                data=serializers.ClaimInvitationUnauthenticatedResponseSerializer(result).data,
...
def create_judge_link(request, competition_id):
    ...
    try:
        ...
        return utils.success_response(
            data=serializers.JudgeLinkResponseSerializer(result, context={'request': request}).data,
            message='Judge link created successfully' if result['created'] else 'Judge link updated',
...
def validate_judge_link(request, token):
    ...
    try:
        result = services.validate_judge_link(
            token=token,
            user=request.user
        )
...
def get_competition_judge_links(request, competition_id):
    ...
    try:
        ...
        return utils.success_response(
            data=result['links'],
            message='Judge links retrieved successfully'
        )
...
def manage_judge_link(request, link_id):
    ...
    if request.method == 'PATCH':
        ...
        try:
            ...
            return utils.success_response(
                data=None,
                message='Judge link deleted successfully'
            )
    ...
    try:
        ...
        return utils.success_response(
            data={
                ...
                'total_count': len(invitations_result['invitations']) + len(links_result['links'])
            },
